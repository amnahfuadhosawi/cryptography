import functools
import collections
import numpy as np

# question 7
c1 = '09e1c5f70a65ac519458e7e53f36'.decode("hex")
msg = 'attack at dawn'
msg2 = 'attack at dusk'

def strxor(u, v):
    return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(u, v)])

def baxor(x,y):
    #xb = bytearray.fromhex(x)
    #yb = bytearray.fromhex(y)
    out = bytearray()
    for (u,v) in zip(x,y):
        out.extend(chr(u^v))
    return out

def hex2ba(x):
    return bytearray.fromhex(x)

ct = ["",
     "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f",
     "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb",
     "",
     "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070",
     "",
     "",
     "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3",
     "",
     "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83",
     "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"]
cba = [hex2ba(x) for x in ct]

# c0 = hex2ba("0"*len(ct[0]))
# z = functools.reduce(baxor, cba, c0)

def computepairwisexor(cba):
    n = len(cba)
    res = {}
    for i in xrange(0, n):
        for j in xrange(i+1, n):
            res[(i,j)] = baxor(cba[i], cba[j])
    return res

# zhash = collections.defaultdict(int)
# for c in z:
#     zhash[c] += 1

def isalpha(s):
    if isinstance(s, str):
        s = ord(s) 
    if s >= 65 and s <= 122:
        return True
    else:
        return False

def asciiprob():
    n = ord('A') - ord('z') + 1
    bitcount = [0]*8
    for i in xrange(ord('A'), ord('z') + 1):
        c = i
        for j in range(0,8):
            bitcount[j] += c&1
            c = c >> 1
    return bitcount

# g = baxor(cba[0], cba[4])
# for i in range(0,len(g)):
#     if g[i] < 65 or g[i] > 122:
#         g[i]='_' 
# print g

xorpairs = computepairwisexor(cba[0:-1])
numct = len(cba) - 1
minct = len(cba[-1])

# Count the number of alpha (A-Za-z) characters at each position

def countalpha(xorpairs):
    res = {}
    for i in xrange(0, numct):
        res[i] = [0]*minct
        for j in xrange(0, numct):
            if i<j:
                idx = (i,j)
            elif i>j:
                idx = (j,i)
            else:               #  i = j
                continue
            for k in xrange(0, minct):
                if isalpha(xorpairs[idx][k]):
                    res[i][k] += 1
    return res

def candidates(alphacounts):
    res = [[]]*minct
    ntop = 3
    for k,v in alphacounts.iteritems():
        for i,acount in enumerate(v):
            res[i] = sorted(res[i] + [(k,acount)], key=lambda x: x[1], reverse=True)[0:ntop]
    return res

z = countalpha(xorpairs)
cand = candidates(z)

# build potential key with top candidates
def guesskey(cand, ntop):
    res = bytearray()
    for i,entry in enumerate(cand):
        idx = entry[ntop][0]
        # print "i %i" % i
        # print "idx " + repr(idx)
        # print "keyfrag " + repr(cba[idx][i])
        res.extend(chr(cba[idx][i] ^ ord(' ')))
    return res

# manual tweaks
def xorkey(byte, s):
    kg[byte] = chr(kg[byte] ^ ord(s))

def tweakkey(msgidx, byteidx, targetchar):
    kg[byteidx] = chr(cba[msgidx][byteidx] ^ ord(targetchar))

kg = guesskey(cand, 0)

#kg[35] = strxor(kg[35], 'A')
tweakkey(0, 35, 'a')
tweakkey(0, 36, 'n')
tweakkey(2, 50, 'e')
tweakkey(9, 56, 't')
tweakkey(6, 82, 'r')
tweakkey(5, 25, 'y')
tweakkey(5, 26, 'p')
tweakkey(0, 7, 'f')
tweakkey(0, 14, 't')
tweakkey(0, 18, 'n')

# xorkey(35, 'A')
#xorkey(36, strxor('C', 'N'))

for i,x in enumerate(cba):
    print baxor(kg, x)[0:minct]
    # print "%i: %s" % (i, baxor(kg, x)[34:minct])
# og = baxor(kg, cba[-1])



